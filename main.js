...
(function(storyContent) {
	...
	*/
		...
		for(var i=0; i<story.globalTags.length; i++) {
			...
			switch(splitTag.property) {
				...
				case "HISTORY":
					...
					if (history_count > 0) document.getElementById('back_button').style.display = "inline-block";
					else {
						history_count = 0;
	...
	var outerScrollContainer = document.getElementById("outerContainer");
	...
	checkLocalStorage();
	if (!loadGame("auto", true)) continueStory(true);
	...
	function scrollDown(previousBottomEdge) {
		...
		function step(time) {
			...
			outerScrollContainer.scrollTop = ((1.0-lerp)*start) + (lerp*target);
	...
	function saveGame(saveSlot, no_confirm) {
		if (saveSlot === undefined) {
			var el = document.getElementById("save-slot");
	...
	function restoreData(saveData) {
		...
		clearText();
		if (saveData.turnIdx === -1) setVisible(".header", true);
		continueStory(true, saveData.saveText, saveData.saveTags);
		scrollToTop();
	}

	function loadGame(saveSlot, no_confirm) {
		if (saveSlot === undefined) {
			var el = document.getElementById("save-slot");
			saveSlot = el.options[el.selectedIndex].value;
		}
	...
	function clearText() {
		setVisible(".header", false);
		removeAll(":not(.header):not(#title):not(#byline)");
		scrollToTop();
	}
	...
	function scrollToTop() {
		...
		if (clear_on_choice) {
			...
			document.getElementById("outerContainer").scrollTop = 0; // For Safari
		}
	}
